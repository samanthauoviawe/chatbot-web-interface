...
export default function Home() {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setMessages([...messages, { text: res.data.response, type: "bot" }]);
    } catch (error) {
      console.error("Error fetching chatbot response:", error);
      setMessages([...messages, { text: "Error: Could not reach the server.", type: "bot" }]);
    } finally {
  ...
  return (
    <div>
      {/* The button to open the chatbot */}
      <button
      ...
      <div style={{ boxShadow: "0 0 #0000, 0 0 #0000, 0 1px 2px 0 rgb(0 0 0 / 0.05)" }}
        ...
        <div className="pr-4 h-[474px]" style={{ minWidth: "100%", display: "table" }}>
          ...
          {/* Map over the chat messages */}
          {messages.map((msg, idx) => (
            <div key={idx} className="flex gap-3 my-4 text-gray-600 text-sm flex-1">
              <span className="relative flex shrink-0 overflow-hidden rounded-full w-8 h-8">
                <div className="rounded-full bg-gray-100 border p-1">
              ...
              <p className="leading-relaxed"><span className="block font-bold text-gray-700">You </span>{msg.text}</p>
            </div>
          ))}
        </div>
